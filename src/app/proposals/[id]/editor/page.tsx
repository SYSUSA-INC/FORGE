        ...
        <Panel title="OUTLINE" code="TOC" dense>
          <ul>
            {sections.map((s, i) => (
              <li
                key={s.id}
                className={`flex items-center justify-between gap-2 border-b-2 border-ink/20 px-3 py-2 font-mono text-[11px] uppercase tracking-wider ${
        ...
        <section className="brut-card overflow-hidden">
          <header className="flex items-center justify-between border-b-2 border-ink bg-ink px-4 py-2 text-paper">
            <div className="flex items-center gap-3 font-mono text-[11px] font-bold uppercase tracking-[0.2em]">
              ...
              <span className="brut-pill bg-signal text-ink">SAVED · 00:04</span>
            </div>
          ...
          <article className="grid grid-cols-[32px_1fr_40px] font-body">
            ...
            <div className="px-8 py-10 text-[15px] leading-[1.6]">
              <h2 className="!font-display text-2xl font-bold !normal-case tracking-tight">
                3.2 AI Integration Approach
              </h2>
              <p className="mt-4">
              ...
              <div className="relative my-6 border-2 border-ink bg-bone p-4">
                ...
                <p className="pt-2">
                  The <u className="decoration-blood decoration-[3px] underline-offset-2">latency
                  budget</u> between sensor ingestion and recommended course-of-action is
                  bounded at <b>180 ms p95</b>, with a fail-safe revert to deterministic
                  heuristics if inference confidence falls below 0.62. A hardware-enforced
                  kill-switch at the Tactical Action Officer station satisfies the{" "}
              ...
              <div className="mt-4 border-2 border-ink">
                <div className="border-b-2 border-ink bg-ink px-3 py-1.5 font-mono text-[10px] uppercase tracking-widest text-paper">
                  TABLE 3-1 · AI GOVERNANCE CONTROLS
                </div>
          ...
          <Panel title="RAG CONTEXT" code="KB" accent="cobalt">
            <ul className="flex flex-col gap-1 font-mono text-[11px]">
              {[
                ...
                <li key={h.k} className="flex items-center justify-between border-b border-ink/20 py-1.5">
                  <div className="min-w-0">
                    <div className="truncate">{h.t}</div>
                    <div className="text-[9px] uppercase text-ink/60">{h.k}</div>
                  </div>
                  <span className="brut-chip bg-signal">{h.d}</span>
                </li>
              ))}
            </ul>
          </Panel>
...
function Toggle({ label, on }: { label: string; on?: boolean }) {
  return (
    <label className="flex cursor-pointer items-center justify-between border-2 border-ink bg-paper px-2 py-1">
      ...
      <span
        className={`relative h-4 w-8 border-2 border-ink ${
          on ? "bg-signal" : "bg-bone"
        }`}
      >
        <span
