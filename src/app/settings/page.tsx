...
export default function SettingsPage() {
  ...
  const [draft, setDraft] = useState<OrgProfile>(org);
  const [dirty, setDirty] = useState(false);

  useEffect(() => {
    if (!dirty) setDraft(org);
  }, [org, dirty]);
  ...
  function handleSamSyncApplied() {
    setDraft(orgStore.getSnapshot());
    setDirty(false);
  }
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Identity, registrations, compliance posture, and integrations for your organization."
        actions={
          <>
            <button
      ...
      {tab === "organization" && (
        <OrganizationTab
          draft={draft}
          update={update}
          onSamSyncApplied={handleSamSyncApplied}
        />
      )}
  ...
  return (
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
      ...
      <SamGovSyncPanel
        ...
        initialUei={draft.uei}
        onApplied={onSamSyncApplied}
      />
  ...
  async function sync() {
    ...
    try {
      const res = await fetch(
        `/api/samgov/entity?uei=${encodeURIComponent(trimmed)}`,
        { cache: "no-store" },
      );
      ...
      orgStore.applySamGovFields(data.profile);
      onApplied();
      setStatus("ok");
      setMessage(`Imported ${data.profile.name ?? "entity"} from SAM.gov.`);
    } catch (err) {
      setStatus("error");
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 md:flex-row md:items-end">
        ...
        >
          {status === "loading" ? "Syncing…" : "Sync from SAM.gov"}
        </button>
      </div>
      {message ? (
